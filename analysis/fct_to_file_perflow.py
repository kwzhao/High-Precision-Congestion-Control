...
def parse_log_entry(line):
    ...
    if (
        not data_pkt
        or node != target_node_id
        or event_type not in ["Enqu", "Dequ"]
        or queue_event not in [1, 2]
    ):
        return None
    else:
...
def calculate_throughput_and_delay(log_file):
    ...
    with open(log_file, "r") as file:
        for line in file:
            ...
            if entry:
                assert (
                    entry["node"] == target_node_id
                    and entry["port"] == target_node_id
                    and entry["queue"] in [1, 3]
                )
                ...
                if entry["event_type"] == "Enqu":
                    ...
                    if enqu_timestamps:
                        queuing_delay_list.append(
                            entry["timestamp"] - enqu_timestamps.popleft()
                        )

    total_time = end_time - start_time
    throughput = total_payload * 8 / total_time
    average_queuing_delay = (
        np.mean(queuing_delay_list) if len(queuing_delay_list) > 0 else 0
    )
...
def calculate_busy_period(log_file):
    ...
    with open(log_file, "r") as file:
        for line in file:
            ...
            if entry:
                ...
                if queue_event == int(
                    QueueEvent.ARRIVAL_FIRST_PKT.value
                ) or queue_event == int(QueueEvent.ARRIVAL_LAST_PKT.value):
                    n_flow_event += 1
                    if n_active_flows == 0:
    ...
    if len(flow_id_per_period_est) > 0:
        n_flows_per_period_est = [
            len(flow_id_per_period_est[i]) for i in range(len(flow_id_per_period_est))
        ]
        ...
        n_flows_per_period_est = [
            len(busy_periods[i]) for i in range(len(busy_periods))
        ]

        flow_id_per_period_unique = [
            item for sublist in busy_periods for item in sublist
        ]
        assert len(flow_id_per_period_unique) == len(set(flow_id_per_period_unique))

        print(
            f"n_flow_event: {n_flow_event}, {len(n_flows_per_period_est)} busy periods, n_flows_per_period_est: {np.min(n_flows_per_period_est)}, {np.max(n_flows_per_period_est)}, {len(flow_id_per_period_unique)} unique flows"
        )
    else:
...
def calculate_busy_period_path(
    ...
    print(
        f"n_flow_event: {len(events)}, {len(busy_periods)} busy periods, flow_size_threshold: {flow_size_threshold}, n_flows_per_period_est: {np.min(busy_periods_len)}, {np.median(busy_periods_len)}, {np.max(busy_periods_len)}"
    )
