...
if __name__ == "__main__":
	...
	if not os.path.exists("%s/stats.npy"%(output_dir)):
		...
		BDP = BDP_DICT[nhost]
		avg_size_base_in_bit = BDP//10*BYTE_TO_BIT # 10KB
		...
		host_list = []
		for i in range(ntc):
			host_list.append((base_t, i, 0))
		heapq.heapify(host_list)
	
		f_sizes_in_byte_list=[]
		f_arr_in_ns_list=[]
		...
		load_range=[0.2,0.8]
		load_bottleneck_range=[0.2,0.8]
		
		size_dist_candidates=np.random.choice(size_distribution_list,size=ntc,replace=True)
		size_sigma_candidates=np.random.rand(ntc)*(size_sigma_range[1]-size_sigma_range[0])+size_sigma_range[0]
		ias_sigma_candidates=np.random.rand(ntc)*(ias_sigma_range[1]-ias_sigma_range[0])+ias_sigma_range[0]
	
		load_candidates=np.random.rand(ntc)*(load_range[1]-load_range[0])+load_range[0]
		...
		load_bottleneck_cur=np.max(list(load_per_link.values()))
		load_candidates=load_candidates*load_bottleneck_target/load_bottleneck_cur
		print("load_bottleneck: ", load_bottleneck_cur,load_bottleneck_target)
		
		n_flows_tmp=n_flows*20
		for tc_idx in range(ntc):
			size_distribution=size_dist_candidates[tc_idx]
			size_sigma_ori = size_sigma_candidates[tc_idx]
			ias_sigma_ori = ias_sigma_candidates[tc_idx]
			load = load_candidates[tc_idx]
			src_dst_pair=host_pair_list[tc_idx]
			if size_distribution=="exp":
				...
				sys.exit(0)
			f_sizes_in_byte_list.append(f_sizes_in_byte)
			avg_in_byte = np.mean(f_sizes_in_byte) 
		
			if ia_distribution=="lognorm":
				avg_inter_arrival_in_s = 1/(host_pair_to_bandwidth_bottleneck_dict[src_dst_pair]*load/8./avg_in_byte)
		...
		flow_id_total=0
		while (n_flows_list[0]<n_flows):
			t,host_pair_idx,flow_idx = host_list[0]
			...
			if flow_idx+2==n_flows_tmp:
				heapq.heapreplace(host_list, (t + inter_t, host_pair_idx,0))
			else:
				heapq.heapreplace(host_list, (t + inter_t, host_pair_idx,flow_idx+1))
			n_flows_list[host_pair_idx]+=1
			flow_id_total+=1
		n_flows_total=np.sum(n_flows_list)
		t+=UNIT_G
		# ofile.write("%f\n"%((t)/1e9))
		data+="%f"%((t)/1e9)
		data = "{}{}".format("%d\n"%n_flows_total,data)
		...
		for i in range(ntc):
			n_flows_done= min(n_flows_list[i],n_flows_tmp)
			end_time=float(np.sum(f_arr_in_ns_list[i][: n_flows_done])) / UNIT_G
			utilization = np.sum(f_sizes_in_byte_list[i][: n_flows_done])*BYTE_TO_BIT / end_time / host_pair_to_bandwidth_bottleneck_dict[host_pair_list[i]]
			print("utilization: ",np.round(utilization, 3), host_pair_list[i], n_flows_list[i], np.round(load_candidates[i], 3), size_dist_candidates[i], size_sigma_candidates[i], ias_sigma_candidates[i],end_time)
		stats={
			"n_flows": n_flows_total,
			"load_bottleneck_target":load_bottleneck_target,
			"host_pair_list":host_pair_list,
			"load_candidates":load_candidates,
			"size_dist_candidates":size_dist_candidates,
			"size_sigma_candidates":size_sigma_candidates,
			"ias_sigma_candidates":ias_sigma_candidates,
		}
