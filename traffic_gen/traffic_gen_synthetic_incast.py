...
if __name__ == "__main__":
    ...
    if not os.path.exists("%s/flows.txt"%(output_dir)):
        ...
        if enable_const:
            f_sizes_in_byte_base=np.ones(n_flows)*constfsize # Byte
            f_sizes_in_byte_base=f_sizes_in_byte_base.astype("int64")
            
            n_flows_tmp=100*n_flows*(ntc-1)+1
            ...
            if size_dist_candidate=="exp":
                ...
                sys.exit(0)
            f_sizes_in_byte=np.concatenate([f_sizes_in_byte_base,f_sizes_in_byte])
        else:
        ...
        if enable_const:
            f_arr_in_ns_base= np.zeros(n_flows).astype("int64")*UNIT_G
            ...
            f_arr_in_ns=np.concatenate([f_arr_in_ns_base,f_arr_in_ns])
        ...
        while (flow_id_total<n_flows_tmp-1):
            if enable_const:
                ...
                if flow_id_total<n_flows:
                    host_pair_idx=host_pair_list_idx[0]
                else:
                    host_pair_idx=host_pair_list_idx[1]
            else:
        ...
        if not enable_const:
            ...
            n_flows_done= min(n_flows_total,n_flows+n_flows_tmp-1)
